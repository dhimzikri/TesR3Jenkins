...
@Component({
  ...
  styleUrls: ['./asli-ri-req.component.css']
})
export class AsliRiReqComponent implements OnInit {
  ...
  @Output() reqAddTrxSrcDataForAsliRIObjOutput = new EventEmitter<ReqAddTrxSrcDataForAsliRIObj>();
  parent: any;
  Addr: any;
  address: string;
  MrCustModelName: any;
  IDType: any;
  reqAddTrxSrcDataForAsliRIObj: ReqAddTrxSrcDataForAsliRIObj;
  ...
  patchObj()
  {
    ...
    if(this.isProfessionalVerification && this.isSelfieAvailable && this.isSelfieValid)
    {
      this.reqAddTrxSrcDataForAsliRIObj.ListReqVerificationType.push(CommonConstant.ASLI_RI_PROF)
    }
    ...
    {
      this.reqAddTrxSrcDataForAsliRIObj.ListReqVerificationType.push(CommonConstant.ASLI_RI_TAX_COY)
    }
