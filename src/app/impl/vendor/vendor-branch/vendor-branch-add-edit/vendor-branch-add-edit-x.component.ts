...
export class VendorBranchAddEditXComponent implements OnInit {
  ...
  showEstablishmentDate: boolean = true;
  ...
  async ngOnInit() {
    ...
    const formattedDate = formatDate(this.businessDt, 'yyyy-MM-dd', 'en-US');

    this.VendorForm.patchValue({
      PartnershipDt: formattedDate
    });

    if (this.MrVendorCategoryCode === CommonConstant.SUPPLIER) {
      this.VendorForm.controls.PartnershipDt.disable();
    }

    if (this.mode == "edit") {
      this.VendorForm.controls.VendorCode.disable();
      await this.getData();
      await this.getVendorX();
      if (this.MrVendorCategoryCode == "ASSET_INSCO_BRANCH") {
        if (this.result.VendorObj.VendorParentId != null) {
          await this.getVendorX(this.result.VendorObj.VendorParentId);
        }
      }
      if (this.MrVendorCategoryCode === CommonConstant.NOTARY_PERSONAL || this.MrVendorCategoryCode === CommonConstant.NOTARY_COMPANY || this.MrVendorCategoryCode === CommonConstant.SUPPLIER) {
        this.VendorForm.controls.Email.setValidators([Validators.required, Validators.pattern(CommonConstant.regexMultipleEmail)]);
      }
    }
    else {
      if (this.MrVendorCategoryCode == CommonConstant.SUPPLIER) {
      ...
      else if (this.MrVendorCategoryCode === CommonConstant.NOTARY_PERSONAL || this.MrVendorCategoryCode === CommonConstant.NOTARY_COMPANY) {
        this.VendorForm.controls.Email.setValidators([Validators.required, Validators.pattern(CommonConstant.regexMultipleEmail)]);
      }
    ...
    for (const vendorAttr of this.VendorAttrList) {
      if (vendorAttr["IsMandatory"] == true) {
        this.VendorForm.controls.VendorAttrList['controls'][vendorAttr["AttrCode"]]['controls'].VendorAttrValue.setValidators(Validators.required);
      }
    }
  }
    ...
    await this.http.post(this.UrlConstantNew.GetRefMasterListKeyValueActiveByCode, refMasterCategoryObj).toPromise().then(
      (response) => {
        ...
        if (this.itemCategoryType.length > 0) {
          if (this.MrVendorCategoryCode == CommonConstant.VENDOR_CATEGORY_GENERAL) {
            this.VendorForm.patchValue({
            ...
            this.VendorForm.patchValue({
              MrVendorCategoryCode: this.MrVendorCategoryCode == CommonConstant.NOTARY_PERSONAL || this.MrVendorCategoryCode == CommonConstant.NOTARY_COMPANY ? CommonConstant.NOTARY : this.MrVendorCategoryCode
            });
          }
        }
      }
  ...
  async getLookupParent(event) {
    this.VendorForm.patchValue({
      ...
      IsSyariah: event.IsSyariah ? 'YES' : 'NO'
    });
    if (this.MrVendorCategoryCode == "ASSET_INSCO_BRANCH") {
      this.isSyariahHidden = false;
      await this.getVendorX(event.VendorId);
    }
  }
  ...
  setLookup() {
    ...
    if (this.MrVendorCategoryCode == "ASSET_INSCO_BRANCH") {
      this.inputLookupParentObj.urlJson = "./assets/impl/uclookup/vendor/lookupAssetInsHOX.json";
      this.inputLookupParentObj.pagingJson = "./assets/impl/uclookup/vendor/lookupAssetInsHOX.json";
      this.inputLookupParentObj.genericJson = "./assets/impl/uclookup/vendor/lookupAssetInsHOX.json";
  ...
  async SaveForm() {
    if (Date.parse(this.VendorForm.controls.EstablishmentDt.value) > Date.parse(formatDate(this.businessDt, 'yyyy-MM-dd', 'en-US'))) {
      ...
      if (this.showEstablishmentDate == false) {
        this.vendorBranchObj.VendorObj.EstablishmentDt = this.VendorForm.controls.PartnershipDt.value;
      }
    ...
    if (
      this.MrVendorCategoryCode == CommonConstant.SUPPLIER &&
      this.VendorForm.controls.IsNpwpExist.value == true
    ) {
      await this.http
        .post(this.UrlConstantNew.GetVendorByTaxNoX, {
          VendorId: vendorAttr.VendorId,
          TaxNo: this.VendorForm.controls.TaxIdNo.value,
        ...
        .then((response) => {
          ...
          if (res !== null) {
            ...
            if (confirmation == false || res.VendorId == this.VendorId) {
              continueToSave = 0;
            }
          }
        });
    ...
    if (continueToSave == 1) {
      if (this.mode == "edit") {
        ...
        let reqEditVendorBranchObjX: any = {
          skuNo: this.VendorForm.controls.SKUNo.value,
          vendorBranchObj: this.vendorBranchObj,
        };
        this.http.post<GenericObj>(this.UrlConstantNew.EditVendorBranchX, reqEditVendorBranchObjX, AdInsConstant.SpinnerOptions).subscribe(
          (response) => {
            this.toastr.successMessage(response["message"]);
            AdInsHelper.RedirectUrl(this.ngxRouter, [NavigationConstant.VENDOR_BRANCH_REG_X], { "VendorId": response.Id, "mode": "edit" });
          });
      }
      else {
        ...
        this.http.post<GenericObj>(this.UrlConstantNew.AddVendorBranchX, reqAddVendorBranchObjX, AdInsConstant.SpinnerOptions).subscribe(
          (response) => {
            this.toastr.successMessage(response["message"]);
            AdInsHelper.RedirectUrl(this.ngxRouter, [NavigationConstant.VENDOR_BRANCH_REG_X], { "VendorId": response.Id });
          });
      }
    }
  ...
  Back() {
    if (this.mode == "edit") {
      AdInsHelper.RedirectUrl(this.ngxRouter, [NavigationConstant.VENDOR_BRANCH_REG_X], { "VendorId": this.VendorId, "MrVendorCategoryCode": this.MrVendorCategoryCode });
    } else {
  ...
  bindText() {
    if (this.MrVendorCategoryCode == CommonConstant.SUPPLIER) {
      ...
      this.Types = "Notary Type";
      this.showEstablishmentDate = false;
      this.VendorForm.controls.EstablishmentDt.clearValidators();
    }
      ...
      this.Code = "Insco Branch Code";
      this.Name = "Insco Branch Name";
      this.Types = "Insco Branch Type";
    }
    else {
      this.Registration = "BRANCH REGISTRATION";
  ...
  async checkAssetSold(isInit: boolean = false) {
    ...
    if (this.VendorForm.controls.ReservedField4.value != "") {
      this.ReservedField4 = this.VendorForm.controls.ReservedField4.value;
    }
    let reqMasterObj: ReqRefMasterByTypeCodeAndMasterCodeObj = {
      MasterCode: this.ReservedField4,
      RefMasterTypeCode: CommonConstant.RefMasterTypeCodeSupplierClass,
    };
    await this.http
      .post(
  ...
  async getVendorX(vendorId: number = 0) {
    if (vendorId == 0) {
      vendorId = this.VendorId
    }
    let result2: any;
    await this.http
      .post(this.UrlConstantNew.GetVendorX, {
        Id: vendorId,
      })
      .toPromise()
      .then(async (response) => {
        result2 = response[CommonConstant.ReturnObj];
      });
    if (result2 != null) {
      this.VendorForm.patchValue({
        SKUNo: result2.SKUNo,
        IsSyariah: result2.IsSyariah ? 'YES' : 'NO'
      });
      if (this.MrVendorCategoryCode == "ASSET_INSCO_BRANCH") {
        this.isSyariahHidden = false;
      }
