...
export function saveListAuthForm(RefRoleId: Number, listTemp: any[], api: any, http: HttpClient, toastr: NGXToastrService, router: Router) {
    ...
    for (var i = 0; i < listTemp.length; i++) {
        ...
        AuthObj.RefRoleId = RefRoleId;
        AuthObj.RefFormId = listTemp[i].RefFormId
    ...
    http.post(url, reqObj, AdInsConstant.SpinnerOptions).subscribe(
      response => {
        ...
        AdInsHelper.RedirectUrl(router,[NavigationConstant.CUSTOM_SYSTEM_SETTING_ROLE_FORM],{ "RefRoleId": RefRoleId });
      }
    );
}